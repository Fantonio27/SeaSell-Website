...
<tbody>
   <?php 
    ...
    $sql = "(SELECT PRODUCT_NO, PRODUCT_NAME, PRODUCT_PRICE, PRODUCT_CONDITION, PRODUCT_DESCRIPTION, PRODUCT_TYPE, PROD_ID FROM fashion_product WHERE STATUS = 'PENDING') 
            UNION
            (SELECT PRODUCT_NO, PRODUCT_NAME, PRODUCT_PRICE, PRODUCT_CONDITION, PRODUCT_DESCRIPTION, PRODUCT_TYPE, PROD_ID FROM bike_product WHERE STATUS = 'PENDING') ORDER BY 'DATE' DESC;";
    ...
    while($row = $result->fetch_assoc()){
        ...
        <td>$row[PROD_ID]</td>
        <td>
            <a class='icon-3' href='listeditem.php?id=$row[PROD_ID]'>Listed</a>
        </td>
        
        <td>
            <a class='icon-1' href='delisted.php?id=$row[PROD_ID]'>Delisted</a>
        </td> 
        </tr>
        ";
    }
